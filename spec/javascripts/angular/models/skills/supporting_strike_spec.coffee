...
describe 'Skills.SupportingStrike', ->
  beforeEach ->
    @prepareSkillApis()
    ...
    @skill = new Skills.SupportingStrike(fixtures.weakening_strike)
  ...
  describe 'on hit', ->
    ...
    it 'in case of hit adds mod(WIS) temporary hits', ->
      @grid = new Grid()
      ...
      expect(@character.i.tempHp).toEqual(@character.mod('wis'))

    it 'does not stack buff', ->
      @grid = new Grid()
      @grid.place(@character, {x: 0, y: 0})
      @grid.place(@monster, {x: 1, y: 1})
      @character.addSkill(@skill)      
